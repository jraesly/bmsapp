...
<?php echo $_L['status'];?>                     </th>
    ...
    $page=$page_num;
    $pagination = "";
    $stmt = $dbh->prepare("SELECT COUNT(id) from tickets where userid='$cid'");
    $stmt->execute();
    $result = $stmt->fetch();
    $totalReq = $result[0];
    $i=0;
    ...
    if($lastpage > 1)
    {
        ...
        //$gen=array("startpoint"=>$startpoint,"limit"=>$limit,"contents"=>$pagination);
           ...
           // $startpoint= '0';
            //$limit= '20';
            $query = "SELECT * from tickets where userid=$cid ORDER BY id DESC LIMIT
{$startpoint} , {$limit}";
...
$status = $value['status'];
          ...
          <td>$i</td><td>$date</td><td>$id</td><td>$tid</td><td>$subject</td>
          <td>$status</td>
...
echo "<tr>
        <td colspan='6'>No Tickets Found For this Client</td></tr>";
            }
            ?>
            </tbody>
        </table>
