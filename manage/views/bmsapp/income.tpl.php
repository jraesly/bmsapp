...
<div class="span4 well">
  	<legend><span id="stTxt"><?php echo $_L['incomeEntry'];?></span></legend>
            ...
            <div class="controls">
                <select name="trfrom"  data-placeholder="Choose an Account" id="trfrom" class="chzn-select" tabindex="2">
                    <option value=""><?php echo $_L['chooseAccount'];?></option>
                     <?php
$query = "SELECT id, name, company from accounts WHERE (acctype='income') ORDER BY id ASC";
        ...
        <div class="control-group">
            ...
            <div class="controls">
              <select name="trto"  data-placeholder="Choose an Account" id="trto" class="chzn-select" tabindex="2">
                    <option value=""><?php echo $_L['chooseAccount'];?></option>
                     <?php
$query = "SELECT id, name, company from accounts WHERE (acctype='Bank' OR acctype='Cash') ORDER BY id ASC";
$stmt = $dbh->prepare("$query");
$stmt->execute();
$result = $stmt->fetchAll();
$i="0";
...
?>
  ...
  <input name="trtype" id="trtype" type="hidden" value="Income" />
...
$tr = ORM::for_table('transactions')->where('ttype','Income')->limit($limit)->offset($startpoint)->order_by_desc('id')->find_many();
